...
export class TransactionsListComponent implements OnInit, OnChanges, OnDestroy {
  ...
  transactionGroups: { status: Status; first: boolean }[] = [];
      ...
      this.refreshOutspends$
        .pipe(
          ...
          tap(({ txIds, outspends }: { txIds: string[]; outspends: Outspend[][] }) => {
            ...
            const transactions = new Map(this.transactions.map(tx => [tx.txid, tx]));
            outspends.forEach((outspend, i) => {
              const tx = transactions.get(txIds[i]);
              if (tx) {
  ...
  ngOnChanges(changes): void {
    if (changes.transactions || changes.groupByBlock || changes.transactionPage) {
  ...
  private updateTransactionGroups(): void {
    ...
    if (!this.groupByBlock || this.transactionPage || !this.transactions?.length) {
      return;
    }

    let previous: Status | null = null;
    this.transactionGroups = this.transactions.map(({ status }) => {
